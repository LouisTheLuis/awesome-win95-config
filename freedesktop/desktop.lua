...
local utils = require("freedesktop.new_utils")
...
local desktop = {
    ...
    baseicons = {
        [1] = {
            label = "Refresh",
            icon  = "drive-harddisk",
            onclick = function () awesome.restart() end
        },
        [2] = {
            label = "This PC",
            icon  = "computer",
...
function desktop.add_single_icon(args, label, icon, onclick)
    ...
    local common = { screen = s, bg = "#00000000", visible = true, type = "desktop", fg = "#ffffff" }
...
function desktop.add_base_icons(args)
    for _,base in ipairs(args.baseicons) do
        desktop.add_single_icon(args, base.label, utils.lookup_icon(base.icon), function()
            if type(base.onclick) == "function" then
                base.onclick()
            else
                awful.spawn.with_shell(string.format("%s '%s'", args.explorer, base.onclick))
            end
        end)
    end
end
...
function desktop.add_icons(args)
    ...
    args.explorer   = args.explorer or "nemo"
    ...
    if not theme.icon_theme then
        theme.icon_theme = args.icon_theme or "Chicago95,"
    end
